...
def compute_planet_longitudes(
    ...
    for dt_ist in dates:
        ...
        for name, (pid, _) in PLANETS.items():
            if name == "Ketu":
                # Ketu is always exactly 180° opposite Rahu on the ecliptic
...
for name in plot_order:
    ...
    fig.add_trace(
        go.Scatter(
            ...
            legendgroup="planets",
            hovertemplate=f"<b>{name}</b><br>%{{x|%d %b %Y %H:%M}}<br>lon %{{y:.3f}}°<extra></extra>",
        ),
        row=2, col=1,
    )
...
with st.expander("Raw data"):
    t1, t2 = st.tabs(["Prices", "Planet longitudes"])
...
st.caption(
    ...
    "Ketu longitude is always Rahu + 180° (mod 360°)."
)
